...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([title isEqualToString:@"出生年月选择器"]) {
        ...
        NSArray *defaultSelValueArr = @[@"第3周",@"第4天"];
        CGXPickerViewManager *mg = [CGXPickerViewManager new];
        mg.pickerTitleSize = 20;
        mg.pickerSelectTitleColor = [UIColor blueColor];
        mg.pickerTitleFont = [UIFont fontWithName:@"PingFangSC-Medium" size:20];
        mg.titleFont = [UIFont fontWithName:@"PingFangSC-Medium" size:16];
        mg.leftBtnFont = [UIFont fontWithName:@"PingFangSC-Medium" size:16];
        mg.rightBtnFont = [UIFont fontWithName:@"PingFangSC-Medium" size:16];
        [CGXPickerView showStringPickerWithTitle:@"学历" DataSource:dataSources DefaultSelValue:nil IsAutoSelect:NO Manager:mg ResultBlock:^(id selectValue, id selectRow) {
            NSLog(@"%@",selectValue); ;
